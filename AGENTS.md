...
## Development Workflow & Documentation (Updated June 2026)

- **CHANGELOG.md** at repo root documents all major merges (especially the June 4 security, encryption-at-rest, GPS audit, and SOS geofence improvements).
- Use the `guardianforge` skill (v3) for deep architecture, security, GPS/PostGIS, AI Guardian, or large refactors. It is synchronized with the live codebase.
- Use focused skills for specialized domains:
  - `postgis-gps-tracking` → GPS, PostGIS `location_history`, high-frequency SOS streaming, geofencing
  - `firebase-functions-brgy-tanod` → Cloud Functions, custom claims role assignment, notifications
  - `offline-sync-dexie` → Dexie queuing, Background Sync, offline SOS/location flows
  - `webllm-guardian-ai` → Local WebLLM Guardian, voice activation, microphone sharing, hybrid fallback
- Always cross-reference `GPS_Audit_Report.md`, `firestore.rules`, and recent commit messages for the current state.
- Maintain strict adherence to offline-first, role-from-claims-only, lazy initialization, and emergency flow resilience principles.
...
const TrackingStatusPanel: React.FC = () => {
  ...
  const [status, setStatus] = useState<"active" | "weak" | "inactive">(
    "inactive"
  );
  ...
  const handleToggleManualGuard = async () => {
    if (manualOverrideActive) {
      ...
      toast.success(
        "Battery Guard released. Standard energy optimization resumed."
      );
    } else {
      ...
      if (success) {
        ...
        toast.success(
          "🛡️ Tactical Wake Lock forced active (Device CPU locked awake)."
        );
      } else {
    ...
    <div className="fixed bottom-24 left-4 right-4 z-[9998] bg-[#0E1116]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-[0_0_30px_rgba(0,0,0,0.6)]">
      <div className="flex items-center justify-between border-b border-white/5 pb-2.5 mb-2.5">
        ...
        <div className="text-right text-[10px] font-mono text-white/40">
          ...
          {accuracy && (
            <span className="ml-2">±{accuracy.toFixed(0)}M ACCURACY</span>
          )}
        </div>
      </div>
          ...
          <div>
            <div
              className={`text-[10px] font-bold font-mono tracking-wide ${
                currentGuardActive ? "text-emerald-400" : "text-zinc-400"
              }`}
            >
              {currentGuardActive
                ? "BATTERY GUARD: ENGAGED [WAKE LOCK ACTIVE]"
                : "BATTERY GUARD: STANDBY MODE"}
            </div>
            <div className="text-[8px] text-white/30 uppercase tracking-widest font-mono mt-0.5">
              {currentGuardActive
                ? "THREAD CORES ACTIVE - SLEEPLOCK PREVENTED"
                : "ENGAGES AUTOMATICALLY ON EMERGENCIES"}
            </div>
          </div>
        </div>

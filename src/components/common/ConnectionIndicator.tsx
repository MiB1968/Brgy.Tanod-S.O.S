...
export const ConnectionIndicator: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const overallStatus = isOnline && isSocketConnected && isFirebaseLive ? 'optimal' : 
                       isOnline ? 'degraded' : 'offline';

  return (
    <div className={cn("bg-black/40 backdrop-blur-xl border border-white/5 rounded-2xl p-4 flex flex-col gap-3 shadow-2xl", className)}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <div className={cn(
            "w-2 h-2 rounded-full",
            overallStatus === 'optimal' ? "bg-success animate-pulse shadow-[0_0_10px_#22c55e]" :
            overallStatus === 'degraded' ? "bg-caution animate-bounce shadow-[0_0_10px_#f59e0b]" :
            "bg-emergency animate-ping shadow-[0_0_10px_#ef4444]"
          )} />
          <span className="text-[10px] font-black font-mono text-white/40 uppercase tracking-widest">Tactical Link Status</span>
        </div>
        ...
        >
          <RefreshCw className={cn("w-3 h-3 text-white/30", checking && "animate-spin text-info")} />
        </button>
      </div>
...
const StatusPill = ({ label, active, icon: Icon, color }: any) => (
  <div className="flex flex-col items-center gap-1.5 p-2 bg-white/[0.02] border border-white/5 rounded-xl">
    ...
    <span className="text-[7px] font-black font-mono text-white/20 uppercase tracking-tight">{label}</span>
  </div>
);

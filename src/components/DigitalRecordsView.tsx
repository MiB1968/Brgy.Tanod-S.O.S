...
import { FileText, ClipboardList, Plus, ExternalLink, Loader2, CheckCircle2 } from 'lucide-react';
import { docsService, formsService } from '../services/googleWorkspaceService';
import { toast } from 'react-hot-toast';

const DigitalRecordsView: React.FC = () => {
  const [isCreatingDoc, setIsCreatingDoc] = useState(false);
  const [isCreatingForm, setIsCreatingForm] = useState(false);
  const [lastCreated, setLastCreated] = useState<{ type: 'doc' | 'form', url: string } | null>(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-900/50 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all"
          >
            {isCreatingDoc ? (
        ...
        >
          ...
          <button
            ...
            className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-900/50 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all"
          >
            {isCreatingForm ? (
              <Loader2 className="animate-spin" />
            ) : (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs font-mono">Your {lastCreated.type === 'doc' ? 'Google Doc' : 'Google Form'} is ready for editing.</p>
            </div>
          </div>
          <a 

...
export function TanodActivityLogs() {
  ...
  const filteredLogs = useMemo(() => {
    return activityLogs
      .filter((log) => {
        ...
        const matchesSearch =
          (log.tanodName?.toLowerCase() || "").includes(
  ...
  const stats = useMemo(() => {
    ...
    const sessionsToday = patrolSessions.filter(
    ...
    return {
      ...
      activeUnits: new Set(
        patrolSessions.filter((s) => !s.endTime).map((s) => s.tanodId)
      ).size,
    };
  ...
  const activityTypes = useMemo(() => {
    ...
    return [
      ...
      {
        value: "alert_response",
        label: "SOS",
        count: counts["alert_response"] || 0,
      },
      {
        value: "patrol_marker",
        label: "Patrol",
        count: counts["patrol_marker"] || 0,
      },
      {
        value: "status_change",
        label: "Status",
        count: counts["status_change"] || 0,
      },
    ];
  }, [activityLogs]);
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `BRGY_TANOD_LOGS_${new Date().toISOString().slice(0, 10)}.csv`
    );
        ...
        {[
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2">
              <span className="text-[9px] font-black uppercase text-white/30 tracking-widest font-mono">
                {stat.label}
              </span>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase font-mono hidden md:block">
                Export
              </span>
            </button>
          </div>
        ...
        <div className="glass-panel rounded-[2rem] overflow-hidden border border-white/5 flex flex-col min-h-[600px]">
          <div className="px-6 py-5 border-b border-white/5 bg-gradient-to-r from-brand-bg/50 to-white/5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-black uppercase tracking-[0.1em] text-white font-mono italic leading-none">
                  Operational Intel
                </h3>
            ...
            <div className="bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl">
              <span className="text-[10px] font-mono font-black text-[#FFB800] italic uppercase tracking-[0.1em]">
              ...
              Object.entries(groupedLogs).map(([date, logs]) => (
                <div key={date} className="relative">
                  <div className="sticky top-0 bg-[#161a21]/95 backdrop-blur-md px-5 py-2 z-10 border-y border-white/5 flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emergency/60">
                      {date}
                    </span>
                    <span className="text-[8px] font-black text-white/20">
                      {logs.length} UNIT_ACTIONS
                    </span>
                  </div>
                  <div className="divide-y divide-white/5">
                    {logs.map((log) => (
                      <div
                        key={log.id}
                        className="p-4 hover:bg-white/5 transition-all group flex items-start gap-4 h-[72px]"
                      >
                        <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/5 flex flex-col items-center justify-center shrink-0 group-hover:border-white/20 transition-all">
                          <span className="text-[9px] font-black text-white/30">
                            #{log.tanodId?.slice(-4).toUpperCase()}
                          </span>
                        </div>

                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[12px] font-black text-white italic uppercase truncate">
                              {log.tanodName}
                            </span>
                            <div
                              className={cn(
                                ...
                                log.type === "alert_response" &&
                                  "bg-[#FF4B4B]/10 text-[#FF4B4B] border-[#FF4B4B]/20",
                                log.type === "patrol_marker" &&
                        ...
                        <div className="text-right shrink-0">
                          ...
                          {log.responseTime ? (
                            <span className="text-[8px] font-black text-warning shadow-glow-amber italic mt-1 block">
                              {Math.floor(log.responseTime / 60)}m
                              {log.responseTime % 60}s RESP
                            </span>
                          ) : (
                            <span className="text-[7px] text-white/10 font-black uppercase tracking-[0.2em] mt-1 block italic opacity-0 group-hover:opacity-100 transition-opacity">
                              Passive
                            </span>
                          )}
                        </div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  onClick={() => {
                    ...
                    document
                      .querySelector(".scrollbar-tactical")
                      ?.scrollTo(0, 0);
                  }}
          ...
          <div className="px-6 py-5 border-b border-white/5 bg-gradient-to-r from-brand-bg/50 to-white/5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[8px] font-black text-white/20 uppercase tracking-[0.3em] font-mono mt-1.5">
                  Sector Trajectories
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto max-h-[650px] p-5 space-y-4 scrollbar-tactical">
            {sortedSessions.length === 0 ? (
              ...
              sortedSessions.slice(0, 15).map((session) => (
                <div
                  key={session.id}
                  className="glass-panel border-white/5 rounded-[1.5rem] overflow-hidden group hover:border-info/30 transition-all duration-300"
                >
                  <button
                    onClick={() =>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-[13px] font-black text-white/90 uppercase italic font-mono tracking-tight">
                          {session.tanodName}
                        </h4>
                    ...
                    <div className="flex items-center gap-4 sm:gap-8">
                      <div className="text-right hidden sm:block">
                        <p className="text-[7px] font-mono text-white/20 uppercase font-black mb-1">
                          METRIC_DUR
                        </p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] font-bold text-info font-mono">
                        ...
                        <div className="p-6">
                          <div className="flex items-center justify-between mb-5">
                            <div className="flex items-center gap-2">
                              ...
                              <h5 className="text-[9px] font-black text-white/40 uppercase tracking-[0.3em] font-mono italic">
                                Telemetry Link
                              </h5>
                            </div>
                            <div className="px-2 py-1 bg-info/10 rounded-lg text-[8px] font-black text-info uppercase font-mono">
                              Detailed Analysis
                            </div>
                          </div>
                          ...
                          <div className="space-y-2 relative before:absolute before:left-[3px] before:top-2 before:bottom-2 before:w-[1px] before:bg-white/5">
                            {session.route.length === 0 ? (
                              <div className="py-8 text-center text-[10px] text-white/20 italic font-mono uppercase tracking-widest">
                                Signal_Loss: Telemetry_Unavailable
                              </div>
                            ) : (
                              session.route.map((p, idx) => (
                                <div
                                  key={idx}
                                  className="flex items-center gap-4 text-[9px] font-mono group/point"
                                >
                                  ...
                                  <span className="text-white/20 tracking-tighter shrink-0">
                                    LAT: {p.lat ? p.lat.toFixed(5) : "N/A"} LNG:{" "}
                                    {p.lng ? p.lng.toFixed(5) : "N/A"}
                                  </span>
                                  <div className="flex-1 h-[1px] bg-white/5 mx-2" />
                                  <span className="text-[8px] text-white/10 uppercase italic font-black">
                                    Link_OK
                                  </span>
                                </div>
                              ))

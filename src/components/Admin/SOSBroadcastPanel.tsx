    ...
    <motion.div className="glass-panel border-white/10 rounded-[40px] p-8 overflow-hidden relative group">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h3 className="text-2xl font-black italic tracking-tighter uppercase text-white font-mono">
              Tactical Broadcast Center
            </h3>
            <p className="text-[10px] font-mono text-white/30 uppercase tracking-[0.2em] mt-1">
              Deploy high-priority alerts to all active units and residents
            </p>
          </div>
        </div>
          ...
          <button
            onClick={async () => {
              const message = window.prompt(
                "Enter SOS Broadcast Message (e.g., Extreme Flood Evacuation):"
              );
              if (!message) return;
          ...
          <button
            onClick={async () => {
              try {
                ...
                batchPromises.push(
                  api.system.updateSiren({ sirenActive: false })
                );

...
export default function AuditLogViewer() {
  ...
  if (loading) {
    return <div className="text-white/40 tracking-wider">Loading audit logs...</div>;
  }
    ...
    <div className="bg-darker border border-white/5 rounded-2xl overflow-hidden">
      <div className="p-6 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3 text-white">
          ...
          <h3 className="font-mono text-sm tracking-widest uppercase font-black">System Audit Trail</h3>
        </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 auto-rows-min mb-4">
                  <div className="flex items-center gap-2 text-sm text-white/60 font-mono tracking-wide">
                    ...
                    <span className="truncate">Admin <span className="text-white">UID:{log.admin_id?.substring(0, 8)}</span></span>
                  </div>
                  {log.target_table && (
                    <div className="flex items-center gap-2 text-sm text-white/60 font-mono tracking-wide">
                      <Target className="w-4 h-4 text-amber-400/80" />
                      <span className="truncate">TGT: <span className="text-white">{log.target_table}#{log.target_id?.substring(0, 8)}</span></span>
                    </div>
                  )}
                </div>
                ...
                {log.details && (
                  <div className="bg-black/40 border border-white/5 rounded-xl p-4 font-mono text-xs overflow-x-auto no-scrollbar">
                    <pre className="text-white/60 break-all whitespace-pre-wrap leading-relaxed"><span className="text-tactical-cyan/40 select-none">&gt; </span>{typeof log.details === 'string' ? JSON.stringify(JSON.parse(log.details), null, 2) : JSON.stringify(log.details, null, 2)}</pre>
                  </div>
                )}
              </div>
            </div>

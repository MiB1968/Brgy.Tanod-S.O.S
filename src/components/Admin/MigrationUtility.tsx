...
const MigrationUtility: React.FC = () => {
  ...
  const seedBarangays = async () => {
    ...
    try {
      ...
      await batch.commit();
      log(
        `Successfully seeded ${protocolCount} protocols and ${sampleBarangays.length} barangays into the new project!`,
        "success"
      );
    } catch (error: any) {
  ...
  const deserializeData = (data: any): any => {
    ...
    if (data.__geopoint__)
      return new GeoPoint(data.__geopoint__.lat, data.__geopoint__.lng);
    ...
    <div className="p-6 bg-white rounded-xl border border-gray-100 shadow-sm space-y-6">
      <div className="flex items-center gap-3 border-b pb-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 leading-tight">
            Guardian Migration Tools
          </h2>
          <p className="text-sm text-gray-500">
            Bypass Cloud Policy restrictions for project migration
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-4 border border-blue-100 bg-blue-50/30 rounded-xl space-y-4">
          ...
          <p className="text-xs text-blue-800 leading-relaxed">
            Prepopulates the new project with essential Barangay data and
            emergency protocols from the source code constants.
          </p>
          <button
        ...
        <div className="p-4 border border-amber-100 bg-amber-50/30 rounded-xl space-y-4">
          ...
          <p className="text-xs text-amber-800 leading-relaxed">
            Upload a `firebase-backup-*.json` file to restore your previous data
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-lg p-4">
        ...
        <ul className="text-xs text-gray-600 space-y-2 list-disc pl-4">
          <li>
            Ensure you are logged in with an <strong>Admin account</strong> in
            the target project.
          </li>
          <li>
            This utility works directly in your browser, bypassing Restricted
            Cloud Policies.
          </li>
          <li>
            A backup file is required for "Manual Restore". If you don't have
            one, use "Initial Seeding" to start fresh.
          </li>
        </ul>
      </div>
    </div>

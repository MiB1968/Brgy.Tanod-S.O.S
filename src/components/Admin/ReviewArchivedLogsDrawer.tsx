...
export function ReviewArchivedLogsDrawer({
  profile,
}: {
  profile: User | null;
}) {
  ...
  const [selectedArchive, setSelectedArchive] =
    useState<AuditLogArchive | null>(null);
  ...
  return (
    <>
      ...
      >
        <span>📁</span>
        <span>
          Archived <span className="hidden xs:inline">Logs</span>
        </span>
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          />
          <div
            className="relative w-full max-w-md h-full bg-[#0F172A] border-l border-amber-500/30
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
              <div>
                ...
                <p className="text-white/40 text-xs mt-0.5">
                  Admin access only · Auto-saved daily at 07:00 AM
                </p>
              </div>
              <button
                onClick={() => {
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-white/40 text-xs">
                          Archived{" "}
                          {new Date(archive.archived_at).toLocaleTimeString()} ·{" "}
                          {archive.log_count} entries
                        </p>
                        <div className="flex gap-2 mt-1">
                          <span className="text-green-400 text-xs">
                            ✔ {archive.resolved_count} resolved
                          </span>
                          <span className="text-red-400 text-xs">
                            ⚠ {archive.unresolved_count} open
                          </span>
                        </div>
                      </div>
                      <span className="text-xs text-white/30 bg-white/5 px-2 py-1 rounded-full shrink-0">
                        {archive.archived_by === "system"
                          ? "🤖 Auto"
                          : "👤 Manual"}
                      </span>
                    </div>
      ...
      {selectedArchive && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-4">
          ...
          />
          <div
            className="relative w-full max-w-lg max-h-[80vh] bg-[#0F172A]
              ...
              {selectedArchive.log_entries.map((entry) => (
                ...
                >
                  ...
                  <p className="text-white/50">
                    Tanod: {entry.tanod_assigned ?? "Unassigned"}
                  </p>

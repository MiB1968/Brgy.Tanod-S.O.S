  ...
  const fetchAnalytics = async (silent = false) => {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "[Analytics] Tactical Intelligence Link offline (serving from local cache or standby):",
          err.message || err
        );
      } else {
        console.error(
          "Failed to fetch analytics from Tactical Intelligence API:",
          err
        );
      }
      ...
      if (cached) {
        try {
          ...
          setError(null);
          if (!silent) {
            toast("Intelligence Link offline. Serving from local cache.", {
              icon: "💾",
            });
          }
        } catch (e) {
  ...
  if (error && !data) {
    return (
      <div className="glass-panel p-12 rounded-[40px] border-white/5 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm font-black uppercase text-emergency tracking-widest font-mono mb-6">
          {error}
        </p>
        <button
          onClick={handleRetry}
  ...
  if (loading && !data) {
    return (
      <div className="glass-panel p-12 rounded-[40px] border-white/5 flex flex-col items-center justify-center animate-pulse">
        ...
        <p className="text-[10px] font-black uppercase text-white/20 tracking-[0.2em] font-mono">
          Syncing tactical intel...
        </p>
      </div>
    );
  }
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-2xl font-black italic tracking-tighter uppercase text-white font-mono leading-none">
                Command Analytics
              </h3>
              {data?.isStandby && (
            ...
            <p className="text-[10px] font-mono text-white/30 uppercase tracking-[0.2em] mt-1 text-info">
              Live CockroachLab Intelligence
            </p>
          </div>
        </div>
        <div className="flex gap-4 items-center">
          ...
          <MetricSmall
            ...
            value={overview.verified_residents}
            icon={Shield}
            color="text-success"
          />
          <MetricSmall
            label="Online Patrols"
            value={overview.total_tanods}
            icon={Users}
            color="text-info"
          />
        </div>
          ...
          >
            <div className="glass-panel p-6 rounded-[24px] border-info/30 bg-info/5 relative">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-[10px] font-black tracking-[0.2em] uppercase text-info font-mono mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-8 rounded-[40px] border-white/5 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black text-white/40 mb-8 uppercase tracking-widest font-mono flex items-center gap-2">
            <Activity className="w-3 h-3 text-emergency" /> Emergency
            Distribution
          </p>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <PieChart>
                <Pie
                  ...
                  stroke="none"
                >
                  {alertsByType.map((_: any, index: number) => (
                    <Cell
                      key={`cell-${index}`}
          ...
          <div className="mt-6 flex flex-wrap gap-4">
            {alertsByType.slice(0, 3).map((item: any, idx: number) => (
              <div key={item.type} className="flex items-center gap-2">
                ...
                />
                <span className="text-[9px] font-black text-white/60 uppercase font-mono">
                  {item.type}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="glass-panel p-8 rounded-[40px] border-white/5 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black text-white/40 mb-8 uppercase tracking-widest font-mono flex items-center gap-2">
            <ChartIcon className="w-3 h-3 text-info" /> Incident Response
            Timeline
          </p>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <AreaChart
                data={alertsHistory.map((h: any) => ({
                  ...h,
                  day: new Date(h.day).toLocaleDateString("en-US", {
                    weekday: "short",
                  }),
                }))}
              >
  ...
  return (
    <div className="flex items-center gap-3 px-6 py-3 bg-brand-bg rounded-[24px] border border-white/5">
      ...
      <div className="flex flex-col">
        ...
        <span className="text-[8px] font-black text-white/30 uppercase tracking-tighter font-mono">
          {label}
        </span>
      </div>
    </div>

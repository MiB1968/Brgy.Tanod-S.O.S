...
export function LiveHeatmap() {
  ...
  const getTypeIcon = (type: string) => {
    switch (type.toUpperCase()) {
      ...
      default:
        return <MapPin className="w-4 h-4 text-white/40" />;
    }
  };
  ...
  const centerLat =
    points[0]?.lat && isValidCoord(points[0].lat, points[0].lng)
      ? points[0].lat
      : defaultCenter[0];
  const centerLng =
    points[0]?.lng && isValidCoord(points[0].lat, points[0].lng)
      ? points[0].lng
      : defaultCenter[1];
    ...
    <div className="relative h-full w-full group">
      ...
      <div className="absolute top-4 left-4 z-[400] flex flex-col gap-2 pointer-events-none">
        <div className="glass-panel px-3 py-2 rounded-xl border-white/5 bg-black/60 backdrop-blur-xl">
          <p className="text-[10px] font-black uppercase text-white/40 tracking-widest font-mono">
            Incident Density
          </p>
          <p className="text-xl font-black text-white italic font-mono">
            {points.length}{" "}
            <span className="text-[10px] not-italic text-white/40 font-bold ml-1">
              REPORTS_30D
            </span>
          </p>
        </div>
      ...
      <div className="absolute bottom-4 right-4 z-[400] glass-panel p-3 rounded-xl border-white/5 bg-black/60 backdrop-blur-xl space-y-2 pointer-events-none">
        <p className="text-[8px] font-black uppercase text-white/20 tracking-widest font-mono mb-2">
          Tactical Legend
        </p>
        {["FIRE", "CRIME", "MEDICAL", "FLOOD"].map((t) => (
          <div
            key={t}
            className="flex items-center gap-2 opacity-80 backdrop-blur-sm"
          >
            {getTypeIcon(t)}
            <span className="text-[9px] font-black uppercase text-white/60 font-mono">
              {t}
            </span>
          </div>
        ))}

...
export const TanodUnitStatusList: React.FC<TanodUnitStatusListProps> = ({
  tanods,
  onUpdateStatus,
}) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {tanods.map((tanod) => {
          ...
          const tacticalStatus =
            patrol?.status || (isAlive ? "patrolling" : "offline");
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                >
                  <UserIcon
                    className={cn(
                      "w-5 h-5",
                      isAlive ? "text-success" : "text-white/40"
                    )}
                  />
                </div>
                <div className="min-w-0">
                  <h4 className="font-bold text-white uppercase italic tracking-tighter font-mono truncate">
                    {tanod.name}
                  </h4>
                  <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest font-mono">
                    {tanod.email}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                  ...
                  >
                    {[
                      ...
                      <option
                        key={status}
                        value={status}
                        className="bg-brand-bg text-black"
                      >
                        {status}
                      </option>
                    ))}
                ...
                <div className="flex justify-between items-center text-[9px] text-white/30 font-mono py-1 border-t border-white/5">
                  ...
                  <span>
                    {tanod.activeAlertId ? (
                      <Zap className="w-3 h-3 text-emergency animate-pulse" />
                    ) : (
                      "---"
                    )}
                  </span>

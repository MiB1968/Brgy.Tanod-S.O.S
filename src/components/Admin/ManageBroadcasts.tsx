    ...
    <div className="glass-panel p-6 rounded-3xl space-y-4">
      ...
      {isAdding && (
        <div className="space-y-3 bg-[#252932] p-4 rounded-xl">
          <select
            ...
            className="w-full bg-[#1A1D23] p-2 rounded-lg text-white"
          >
          ...
          <textarea
            ...
            className="w-full bg-[#1A1D23] p-3 rounded-lg text-white text-sm"
          />
          <button
            onClick={handleAdd}
            className="w-full py-2 bg-emergency rounded-lg text-white font-bold"
          >
            SEND BROADCAST
          </button>
        </div>
      ...
      <div className="space-y-2">
        {broadcasts.map((b) => (
          <div
            key={b.id}
            className="flex justify-between items-center bg-white/5 p-3 rounded-lg text-xs"
          >
            <div className="flex items-center gap-2">
              <Globe
                className={`w-3 h-3 ${
                  b.isActive ? "text-green-500" : "text-gray-500"
                }`}
              />
              <span>{b.message}</span>
            </div>
            <button
              onClick={() => toggleBroadcast(b)}
              className={`px-2 py-1 rounded ${
                b.isActive ? "bg-green-500/10" : "bg-gray-500/10"
              }`}
            >

  ...
  return (
    <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:justify-between items-start md:items-end gap-6 mb-10 relative glass-panel p-6 sm:p-8 rounded-[32px] sm:rounded-[48px] border-white/10 skew-card shadow-2xl">
      ...
      <div className="relative z-10 w-full">
        ...
        <p className="text-[8px] sm:text-[10px] font-mono text-white/40 uppercase tracking-[0.2em] sm:tracking-[0.4em] mt-4 bg-white/5 inline-block px-4 py-1.5 rounded-full border border-white/5">Strategic Response Matrix</p>
      </div>

      <div className="flex flex-row items-center gap-2 sm:gap-3 relative z-10 w-full md:w-auto">
        <button
          onClick={() => setIsAboutOpen(true)}
          className="flex-1 flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-xl sm:rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all group hover:border-info/50"
        >

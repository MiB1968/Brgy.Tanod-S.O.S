...
export const ResidentVerification = () => {
  ...
  const getResidentDetails = (raw: any) => {
    ...
    return {
      ...
      emergencyContactName: raw.emergencyContactName || raw.emergency_contact_name || 'N/A',
      emergencyContactPhone: raw.emergencyContactPhone || raw.emergency_contact_phone || 'N/A',
  ...
  const filteredResidents = residents.map(r => getResidentDetails(r)).filter(r => {
    if (!r) return false;
    const matchesFilter = 
      filter === 'all' ? true :
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                    <div className="flex items-center gap-4">
                      ...
                      >
                        {resident.selfieUrl ? (
                          <img src={resident.selfieUrl} className="w-full h-full object-cover" alt="Selfie" />
                        ) : (
                          <Shield className={`w-6 h-6 ${resident.status === 'pending' ? 'text-yellow-500' : 'text-tactical-cyan'}`} />
                        )}
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-black italic text-lg uppercase tracking-tight font-mono hover:text-tactical-cyan cursor-pointer" onClick={() => setSelectedInspect(resident)}>
                            {resident.name}
                          </h4>
                        </div>
                    ...
                    <div className="grid grid-cols-2 lg:flex lg:items-center gap-8">
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] font-bold uppercase truncate max-w-[200px]">{resident.address}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[8px] font-black uppercase text-white/20 tracking-widest">House #</p>
                        <p className="text-[10px] font-bold uppercase">{resident.houseNumber}</p>
                      </div>
                      <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 mt-4 md:mt-0">
                      ...
                      >
                        <Eye className="w-3.5 h-3.5 text-tactical-cyan" /> Inspect
                      </button>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                      <p className="text-[10px] font-black uppercase tracking-widest text-warning font-mono">Captured Visual Proof</p>
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <span className="text-[9px] font-mono tracking-widest text-white/40 uppercase">1. FACE BIOMETRIC</span>
                          <div className="w-full aspect-square rounded-3xl bg-[#0B0D12] overflow-hidden border border-white/10 shadow-xl relative min-h-[180px]">
                            {detailed.selfieUrl ? (
                              <img src={detailed.selfieUrl} className="w-full h-full object-cover" alt="Verification Selfie" />
                            ) : (
                              <div className="absolute inset-0 flex flex-col items-center justify-center text-white/20">
                                <Shield className="w-12 h-12 mb-2 animate-pulse" />
                                <span className="text-[9px] font-mono">NO SELF PORTRAIT</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          <span className="text-[9px] font-mono tracking-widest text-white/40 uppercase">2. GOVERNMENT ID</span>
                          <div className="w-full aspect-square rounded-3xl bg-[#0B0D12] overflow-hidden border border-white/10 shadow-xl relative min-h-[180px] flex flex-col items-center justify-center">
                            {detailed.idPhotoUrl ? (
                              <img src={detailed.idPhotoUrl} className="w-full h-full object-contain" alt="Government ID" />
                            ) : (
                              <div className="p-4 text-center text-white/30 space-y-2">
                                <XCircle className="w-10 h-10 mx-auto text-white/10" />
                                <p className="text-[9px] font-mono leading-none tracking-widest uppercase">NOT ATTACHED</p>
                                <p className="text-[8px] leading-relaxed text-white/20 italic">Validated via community verification</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {detailed.status === 'pending' ? (
                        <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl flex items-start gap-3">
                          ...
                          <div className="space-y-1">
                            <p className="text-[10px] font-black tracking-widest uppercase text-yellow-500 font-mono">Awaiting Clearance</p>
                            <p className="text-[10px] text-white/50 leading-relaxed font-semibold">Inspect and compare the resident name with the face photograph above before providing system clearance.</p>
                          </div>
                        </div>
                      ) : (
                        <div className="p-4 bg-tactical-cyan/10 border border-tactical-cyan/20 rounded-2xl flex items-start gap-3">
                          ...
                          <div className="space-y-1">
                            <p className="text-[10px] font-black tracking-widest uppercase text-tactical-cyan font-mono">Account Authorized</p>
                            <p className="text-[10px] text-white/50 leading-relaxed font-semibold">Verified on {detailed.verificationDate ? new Date(detailed.verificationDate).toLocaleDateString() : 'N/A'}. This resident holds active tactical SOS rights.</p>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-4">
                      <p className="text-[10px] font-black uppercase tracking-widest text-tactical-cyan font-mono">Residential Context & Geolocation</p>
                      
                      <div className="grid grid-cols-2 gap-4 bg-white/[0.02] p-4 rounded-3xl border border-white/5 font-mono">
                        <div className="space-y-1">
                          <span className="text-[8px] text-white/30 uppercase tracking-widest flex items-center gap-1"><Home className="w-3 h-3" /> Household Name</span>
                          <p className="text-xs font-black uppercase text-white truncate">{detailed.name}</p>
                        </div>
                        <div className="space-y-1">
                          <span className="text-[8px] text-white/30 uppercase tracking-widest flex items-center gap-1"><Phone className="w-3 h-3" /> Contact</span>
                          <p className="text-xs font-black uppercase text-white truncate">{detailed.phone}</p>
                        </div>
                        <div className="space-y-1 mt-3 col-span-2">
                          <span className="text-[8px] text-white/30 uppercase tracking-widest flex items-center gap-1"><MapPin className="w-3 h-3" /> Binding Coordinates</span>
                          <p className="text-[10px] font-black text-tactical-cyan">Lat: {detailed.gpsLat.toFixed(5)} • Lng: {detailed.gpsLng.toFixed(5)}</p>
                        </div>
                      </div>
                      ...
                      <div className="h-44 rounded-3xl overflow-hidden border border-white/10 relative shadow-xl">
                        ...
                        >
                          ...
                          />
                          <Marker position={[detailed.gpsLat, detailed.gpsLng]} icon={DefaultIcon} />
                        </MapContainer>
                        <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 text-[8px] font-mono uppercase tracking-[0.2em] text-white/60 pointer-events-none flex items-center gap-1">
                          <MapPin className="w-2.5 h-2.5 text-emergency" /> Pinpoint Location
                        </div>
                      </div>

                      <div className="space-y-1 bg-white/[0.01] p-4 rounded-2xl border border-white/5">
                        <span className="text-[8px] font-mono text-white/30 tracking-widest uppercase">Verified Residential Sector / Address</span>
                        <p className="text-[10px] font-mono uppercase font-black tracking-wide leading-relaxed text-white/80">{detailed.address}</p>
                      </div>
                    </div>
                  </div>
                </div>

...
export const GuardianVoiceAssistant: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isListening) {
      micManager
        .acquire("guardian-voice", "GuardianVoiceAssistant")
        .then((stream) => {
          acquired = true;
        ...
        .catch((err) => {
          ...
          if (
            ...
            toast.error("Tactical analysis requires microphone access.", {
              id: "mic-denied",
            });
          }
        });
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent("voice-assistant-change", {
        detail: { isListening },
      })
    );
  }, [isListening]);
      ...
      >
              ...
              {showChat ? (
                <form onSubmit={handleChatSubmit} className="relative">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <p className="text-[9px] text-blue-400/50 font-mono uppercase tracking-widest">
                ...
                >
                  <div className="relative">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center bg-blue-600/20 text-blue-400 ${
                        isSpeaking ? "animate-pulse" : ""
                      } group-hover:bg-blue-600/35 transition-colors`}
                    >
                      <Bot size={20} />
                    </div>
                    {isSpeaking && (
                      ...
                      {isListening ? (
                        <div className="flex flex-col gap-1 w-full">
                          ...
                          <div className="flex gap-0.5 h-3 items-end">
                            {Array.from(visualData.slice(0, 8)).map(
                              (val, i) => (
                                <motion.div
                                  key={i}
                                  animate={{
                          ...
                          <p className="text-[8px] text-blue-400/40 font-mono uppercase tracking-wider mt-1 group-hover:text-blue-400/70 transition-colors">
                            Tap, Click o Uriin dito
                          </p>
                        </div>
                      ) : (
        ...
        <div className="hidden bg-slate-800">
          ...
          <motion.button
            ...
            ${
              isListening
                ? "bg-red-500/70 ring-4 ring-red-500/20"
                : "bg-sky-500/60 hover:bg-sky-400/70 ring-4 ring-sky-400/20"
            }
            ${
              isDraggable
                ? "cursor-grabbing ring-white scale-110"
                : "cursor-pointer"
            }
          `}
          >
            {/* Inner Glow Core */}
            <div
              className={`absolute inset-0 rounded-full blur-md opacity-50 bg-sky-400 ${
            ...
            <TanodLogo
              ...
              className={isListening ? "animate-pulse" : "relative z-10"}
            />
                ...
                <>
                  <motion.div
                    ...
                    className={`absolute inset-0 border-2 ${
                      isListening ? "border-red-400" : "border-sky-400"

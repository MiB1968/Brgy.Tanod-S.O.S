  ...
  const fetchInvites = async () => {
    try {
      const data = await api.generic.list(
        `witness_invites?witnessUserId=${userId}&status=pending`
      );
      setInvites(data);
    } catch (err) {
        ...
        >
          ...
          <div className="p-8 relative z-10">
            <div className="flex items-start gap-4 mb-8">
              ...
              <div>
                <h3 className="text-2xl font-black text-white italic tracking-tight uppercase leading-none mb-2">
                  Emergency Nearby
                </h3>
                <p className="text-white/40 text-[11px] uppercase font-mono tracking-widest leading-relaxed">
                  A neighbor in your{" "}
                  <span className="text-emergency font-black">
                    Witness Circle
                  </span>{" "}
                  has triggered an SOS. Can you provide eyes on the situation?
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-white/40 uppercase tracking-[0.3em] font-mono mb-1">
                  Incident Proximity
                </span>
                <span className="text-[11px] font-mono font-bold text-white uppercase italic tracking-tighter">
                  APPROX. 500M FROM YOUR POSITION
                </span>
              </div>
            </motion.div>
          ...
          <div className="bg-white/[0.04] px-6 py-4 border-t border-white/5 flex justify-center backdrop-blur-md">
            <span className="text-[8px] font-mono font-black text-white/20 uppercase tracking-[0.5em] animate-pulse">
              SECURE_BRGY_SOS_LINE_CONNECTED
            </span>
          </div>
        </motion.div>
      ))}

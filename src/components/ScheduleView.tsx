...
export default function ScheduleView({
  ...
  return (
    <div className="glass-panel border-white/5 rounded-[48px] p-8 md:p-14 shadow-command max-w-5xl relative overflow-hidden">
      ...
      {loading ? (
        <div className="py-24 text-center animate-pulse text-white/20 font-mono font-bold tracking-widest">
              ...
              >
                <div className="text-center md:text-left flex-1">
                  <div className="flex items-center justify-center md:justify-start gap-4 mb-3">
                    <p
                      className={cn(
                        "text-[10px] font-black tracking-[0.3em] font-mono",
                        isActive ? "text-info" : "text-white/20"
                      )}
                    >
                      T-MINUS {format(new Date(s.startTime), "HH:mm")} -{" "}
                      {format(new Date(s.endTime), "HH:mm")}
                    </p>
                    {isActive && (
                      <span className="flex items-center gap-2">
                        <span className="w-2 h-2 bg-info rounded-full animate-ping shadow-info/50" />
                        <span className="text-[9px] font-black text-info font-mono uppercase tracking-widest">
                          LIVE_STATUS
                        </span>
                      </span>
                    )}
                  </div>
                  <h4 className="text-2xl md:text-4xl font-black tracking-tighter mb-3 italic text-white uppercase font-mono leading-none">
                    {s.sector}
                  </h4>
                  <p className="text-white/40 font-bold uppercase tracking-widest font-mono text-xs flex items-center justify-center md:justify-start gap-3">
                    <UserIcon className="w-5 h-5 text-info/50" /> OFFICER{" "}
                    {s.tanodName.toUpperCase()}
                  </p>
                </div>
                <div
                  className={cn(

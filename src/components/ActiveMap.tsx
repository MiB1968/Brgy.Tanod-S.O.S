      ...
      <div class="relative flex items-center justify-center">
        ...
        <div class="absolute -top-1 -right-1 z-20 w-4 h-4 bg-[${color}] rounded-full border-2 border-[#16191F] flex items-center justify-center">
          <span class="text-[8px] font-black text-white">${
            alert.aiAnalysis?.severityScore || "!"
          }</span>
        </div>
      </div>
    `,
...
export default function ActiveMap({
  ...
  useEffect(() => {
    if (alerts.length > 0) {
      const activeAlert =
        alerts.find((a) => a.status === "pending") || alerts[0];
  ...
  return (
    <div className="w-full h-full rounded-3xl overflow-hidden relative border border-[#2D3139]">
      ...
      >
        ...
        {onLocationSelect &&
          selectionLocation &&
          isValidCoord(selectionLocation.lat, selectionLocation.lng) && (
            <Marker
              ...
              <Popup className="dark-popup">
                <div className="p-1">
                  ...
                  <p className="text-[10px]">
                    Drag this pin to the exact emergency site
                  </p>
                </div>
              </Popup>
            </Marker>
        ...
        {alerts
          ...
          .map((alert) => (
            <React.Fragment key={alert.id}>
              {showHeatmap &&
                alert.aiAnalysis &&
                alert.aiAnalysis.severityScore > 6 && (
                  <Circle
                  ...
                  <div className="p-3 min-w-[200px] space-y-3">
                    <div className="flex items-center justify-between border-b border-white/10 pb-2">
                      <div className="flex flex-col">
                        <p className="font-black text-[#FF4B4B] uppercase text-xs tracking-tighter m-0 leading-tight">
                          CRITICAL SOS
                        </p>
                      ...
                      <div className="bg-[#FF4B4B]/10 border border-[#FF4B4B]/30 px-2 py-1 rounded">
                        <span className="text-[10px] font-black text-[#FF4B4B]">
                          SV: {alert.aiAnalysis?.severityScore || "N/A"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono font-bold uppercase text-white/60 tracking-wider">
                          Type: {alert.type}
                        </span>
                      </div>
                      ...
                      {alert.aiAnalysis?.summary && (
                        <div className="p-2 bg-white/5 rounded-lg border border-white/5">
                          <p className="text-[10px] text-white/80 leading-relaxed font-medium italic">
                            "
                            {alert.aiAnalysis.summary.length > 80
                              ? alert.aiAnalysis.summary.substring(0, 80) +
                                "..."
                              : alert.aiAnalysis.summary}
                            "
                          </p>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        <div className="flex flex-col">
                          <span className="text-[8px] text-white/30 uppercase font-black">
                            Status
                          </span>
                          <span className="text-[10px] text-info font-bold uppercase">
                            {alert.status}
                          </span>
                        </div>
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-white/70 font-mono">
                            ±{Math.round(alert.location.accuracy || 0)}m
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="relative">
                        <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden">
                          <span className="text-[10px] font-black text-white/40">
                            {patrol.tanodName.charAt(0)}
                          </span>
                        </div>
                        <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-success border-2 border-[#16191F] animate-pulse" />
                      </div>
                      <div className="flex flex-col">
                        <p className="font-black text-white uppercase text-xs tracking-tighter m-0 leading-tight">
                          {patrol.tanodName}
                        </p>
                        <p className="text-[8px] font-mono text-white/40 m-0 uppercase tracking-widest">
                          Patrol Officer
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex justify-between items-center text-[10px]">
                        ...
                        <span className="text-white/70 font-mono">
                          STABLE | ±{Math.round(patrol.location.accuracy || 0)}m
                        </span>
                      </div>
      ...
      {/* Tactical Overlay */}
      <div
        className="absolute top-0 right-0 bottom-0 left-0 pointer-events-none z-[400] opacity-20 overflow-hidden"
      ...
      >
        ...
        <div
          className="absolute top-1/2 left-1/2 w-[200%] h-[200%] bg-[conic-gradient(from_0deg,transparent_0deg,rgba(255,75,75,0.1)_90deg,rgba(255,75,75,0.3)_180deg,transparent_180deg)] animate-[radar_10s_linear_infinite]"

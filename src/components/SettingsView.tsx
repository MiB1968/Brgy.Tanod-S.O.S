  ...
  return (
    <div className="max-w-4xl space-y-8">
      <div className="glass-panel p-8 md:p-12 rounded-[48px] border-white/5 shadow-command relative overflow-hidden">
        ...
        <h2 className="text-3xl md:text-4xl font-black italic tracking-tighter uppercase text-white font-mono leading-none">Command Config</h2>
        <p className="text-white/30 font-bold text-xs md:text-sm uppercase tracking-[0.3em] font-mono mt-3">Personal Link Preferences & Privacy Controls</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="glass-panel border-white/5 rounded-[40px] p-8 md:p-10 space-y-8 relative overflow-hidden group hover:border-white/10 transition-all shadow-command skew-card">
           ...
           <div>
             <h4 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mb-4 font-mono">Authentication Profile</h4>
             <div className="flex items-center gap-6">
                <div className="w-20 h-20 rounded-3xl bg-emergency/20 border border-emergency/30 flex items-center justify-center p-0.5">
                   <div className="w-full h-full bg-emergency rounded-2xl flex items-center justify-center shadow-lg">
                      <span className="text-3xl font-black text-white italic">{profile.name.charAt(0)}</span>
                   </div>
                </div>
                <div>
                  <p className="text-2xl font-black text-white italic tracking-tighter uppercase font-mono leading-tight">{profile.name}</p>
                  <p className="text-white/40 font-bold text-[10px] uppercase tracking-widest font-mono mt-1">{profile.email}</p>
           ...
           <div className="space-y-4 pt-6 border-t border-white/5">
             <div className="flex justify-between items-center text-xs font-mono">
                <span className="text-white/30 font-black uppercase tracking-widest">Active Phone Link</span>
                <span className="text-white font-bold italic">{profile.phone || 'NONE_CONNECTED'}</span>
             </div>
             <div className="flex justify-between items-center text-xs font-mono">
                <span className="text-white/30 font-black uppercase tracking-widest">Global Status</span>
                <span className="text-success font-black italic uppercase">Verified_Link</span>
             </div>
           </div>
        </div>
        ...
        <div className="glass-panel border-white/5 rounded-[40px] p-8 md:p-10 space-y-8 shadow-command">
           <h4 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] font-mono">System Parameters</h4>
        ...
        <div className="md:col-span-2 glass-panel border-white/5 rounded-[40px] p-8 md:p-12 shadow-command">
          <h4 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mb-8 font-mono">Cache & Storage Engine</h4>
          ...
          <div className="flex flex-col md:flex-row gap-6">
             ...
             >
                ...
                <h5 className="text-white font-black italic font-mono uppercase text-lg tracking-tighter">Export Session Data</h5>
             ...
             >
                ...
                <h5 className="text-emergency font-black italic font-mono uppercase text-lg tracking-tighter">Terminal Purge</h5>
                <p className="text-emergency/30 text-[10px] font-black uppercase tracking-widest font-mono mt-1">Clear all local caches & syncs</p>
             </button>
          </div>
        </div>
        ...
        <div className="md:col-span-2 p-10 bg-emergency/10 border border-emergency/30 rounded-[40px] flex gap-8 items-center">
           ...
           <div>
              <h5 className="text-xl font-black italic text-white font-mono uppercase tracking-tighter">Operational Security Warning</h5>
              <p className="text-white/40 text-xs md:text-sm font-bold mt-2 font-mono uppercase leading-relaxed max-w-2xl">
                Always ensure your communication link is encrypted. If you lose your primary authenticated device, initiate an immediate remote session kill via Brgy. Command Admin.
              </p>
           </div>
        </div>
      </div>
    ...
    <div className="flex items-center justify-between gap-6 group">
      <div className="flex items-center gap-5">
        <div className={cn(
          ...
          active ? "bg-info/20 text-info border-info/30" : "bg-white/5 text-white/20 border-white/5"
        )}>
          <Icon className="w-5 h-5" />
        </div>
        <div>
          <p className="text-white font-black text-xs uppercase italic tracking-tighter mb-1 font-mono">{label}</p>
          <p className="text-white/20 text-[9px] font-black uppercase tracking-widest font-mono">{desc}</p>
        </div>
      </div>
      ...
      <button 
        ...
        className={cn(
          ...
          active ? "bg-info shadow-[0_0_15px_rgba(14,165,233,0.3)]" : "bg-white/10"
        )}
      >
        <div className={cn(

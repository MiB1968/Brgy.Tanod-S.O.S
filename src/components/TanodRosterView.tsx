    ...
    <div className="space-y-8">
      <div className="glass-panel p-8 md:p-12 rounded-[48px] border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-command">
        <div>
          ...
          <p className="text-white/40 font-bold text-xs md:text-sm uppercase tracking-[0.3em] font-mono mt-3">
            Tactical Peacekeeping Force Inventory
          </p>
        </div>
        <button
        ...
        {addingUnit && (
          <div className="fixed inset-0 bg-brand-bg/90 backdrop-blur-md z-[9999] flex items-center justify-center p-6">
            ...
            >
              ...
              <h3 className="font-black italic text-2xl md:text-3xl tracking-tighter text-white mb-4 uppercase font-mono leading-none">
                Initialize New Unit
              </h3>
              <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.3em] mb-10 font-mono">
                Deploy authorized personnel to the network
              </p>

              <div className="space-y-6 mb-10">
                <div className="space-y-2">
                  <label className="text-[10px] font-black tracking-[0.2em] text-white/30 uppercase mb-2 block font-mono">
                    Operator Identity
                  </label>
                  <input
                    type="text"
            ...
            >
              ...
              <div className="flex items-center gap-6 mb-8">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="text-2xl font-black italic tracking-tighter text-white truncate uppercase font-mono leading-none">
                      {t.name}
                    </h4>
                    {isActuallyActive && (
                      <span className="w-2 h-2 bg-success rounded-full animate-pulse shadow-glow-success" />
                    )}
                  </div>
                  <p className="font-mono text-white/30 text-[9px] uppercase font-bold tracking-[0.2em]">
                    {t.id || `UNIT-${t.id?.slice(0, 4).toUpperCase()}`}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-8">
                <div className="flex justify-between items-center p-4 bg-brand-bg/50 rounded-2xl border border-white/5">
                  <span className="text-[9px] font-black uppercase text-white/20 tracking-[0.2em] font-mono">
                    Duty Status
                  </span>
                ...
                <div className="flex justify-between items-center p-4 bg-brand-bg/50 rounded-2xl border border-white/5">
                  ...
                  <span
                    className={cn(
                      ...
                      isActuallyActive
                        ? patrolMatch?.status === "responding"
                          ? "text-emergency shadow-glow-red"
                          : "text-success"
                        : "text-caution"
                    )}
                  >
              ...
              <div className="flex gap-3">
                <button
                  onClick={() =>
                    toast.success(`Accessing tactical profile for ${t.name}`, {
                      icon: "👮",
                    })
                  }
                ...
                <button
                  onClick={() =>
                    toast.success(
                      `Retrieving operational history for ${t.name}`,
                      { icon: "📋" }
                    )
                  }
                  className="flex-1 py-4 glass-panel border-white/10 text-[9px] font-black uppercase tracking-widest text-white/30 hover:text-white hover:border-white/30 rounded-2xl transition-all font-mono"

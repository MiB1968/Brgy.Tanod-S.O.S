...
export default function PatrolScheduler({ profile }: { profile: any }) {
  ...
  const fetchData = async () => {
    try {
      ...
      setTanods(
        usersData.filter(
          (u: User) => u.status === "approved" || u.status === "Available"
        )
      );
    } catch (err) {
  ...
  const handleUpsertShift = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const shiftData = {
        ...
        notes,
        createdAt: editingShift
          ? editingShift.createdAt
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#16191F] p-6 rounded-[32px] border border-[#2D3139] shadow-xl">
        <div>
          ...
          <p className="text-[#8E9299] text-xs font-medium">
            Manage officer shifts and patrol sectors
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {shifts.map((shift) => (
          ...
          >
            ...
            {shift.notes && (
              <p className="text-xs text-[#8E9299] italic italic">
                "{shift.notes}"
              </p>
            )}
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-[#2D3139] flex justify-between items-center bg-[#1A1D23]">
                <h3 className="font-black italic text-2xl tracking-tighter uppercase">
              ...
              <form onSubmit={handleUpsertShift} className="p-8 space-y-6">
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-black uppercase text-[#8E9299] tracking-widest mb-2 block">
                      Officer
                    </label>
                    <select
                      required

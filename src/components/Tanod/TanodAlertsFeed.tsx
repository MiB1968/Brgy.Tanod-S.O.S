...
export function TanodAlertsFeed({
  alerts,
  profile,
  onUpdateStatus,
  onDetails,
}: TanodAlertsFeedProps) {
  ...
  useEffect(() => {
    ...
    if (alerts.length > prevAlertsLength.current) {
      ...
      speak({
        text: `Bagong alerto natanggap. May ${newAlertsCount} bagong emergency.`,
        language: "en",
      });
    }
  ...
  const filteredAlerts = alerts.filter((alert) => {
    ...
    if (filterType !== "ALL" && !alert.type.toUpperCase().includes(filterType))
      return false;
      ...
      <TacticalCard>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-tactical-cyan/60">
              Real-time Stream
            </span>
          </div>
        </div>
        ...
        {alerts.filter((a) => a.status === "pending").length > 0 && (
          <div className="space-y-4 mb-6">
            <p className="text-[9px] font-black uppercase text-tactical-red tracking-widest pl-2">
              Critical: Pending Dispatch
            </p>
            {alerts
        ...
        <AnimatePresence mode="popLayout">
          {filteredAlerts.length === 0 ? (
            <TacticalCard className="text-center p-12">
              ...
              <p className="text-white/40 font-black uppercase tracking-widest text-xs font-mono">
                  ...
                  <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex-1 space-y-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h4 className="font-black text-lg text-white font-display uppercase italic">
                            {alert.residentName}
                          </h4>
                          <p className="text-[10px] text-tactical-cyan/60 font-bold font-mono uppercase tracking-tight flex items-center gap-1">
                    ...
                    <div className="flex flex-col gap-3 justify-center shrink-0">
                      <a
                        ...
                        target="_blank"
                        className="px-6 py-3 bg-tactical-dark border border-tactical-cyan/30 text-white text-[10px] font-black rounded-xl font-mono text-center hover:bg-tactical-cyan/10"
                      >
                        GPS LINK
                      </a>
                      {isActiveAlert(alert) && (
                        <TacticalButton
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            onUpdateStatus(alert, "responding");
                          }}
                        />
                      )}

...
export function StatusTogglePanel({
  profile,
  sirenActive,
  onToggleSiren,
  updateTanodStatus,
}: StatusTogglePanelProps) {
  ...
  const handleStatusChange = async (newStatus: RegistryStatus) => {
    ...
    try {
      ...
      const isOnline = ["on patrol", "responding", "available"].includes(
  ...
  const toggleLocation = async () => {
    ...
    try {
      ...
      toast.success(
        isLocationEnabled ? "GPS SHARING SUSPENDED" : "GPS SHARING ACTIVE"
      );
    } catch (e) {
  ...
  return (
    <TacticalCard className="mb-8">
      <div className="absolute top-8 right-8 flex items-center justify-center">
        ...
        <span className="relative text-2xl drop-shadow-[0_0_8px_var(--color-tactical-cyan)] animate-pulse">
          🟢
        </span>
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="mb-4 flex items-center justify-between bg-tactical-dark p-4 rounded-2xl border border-tactical-cyan/30 w-fit gap-6">
          <div className="flex items-center gap-2">
            <div
              className={cn(
                ...
                isLocationEnabled
                  ? "bg-tactical-cyan shadow-[0_0_10px_var(--color-tactical-cyan)] animate-pulse"
                  : "bg-white/10"
              )}
            />
          ...
          <TacticalButton
            ...
            className={cn(
              ...
              isLocationEnabled
                ? "bg-tactical-cyan/20 border-tactical-cyan"
                : ""
            )}
          />
        </div>
        ...
        <div className="flex flex-col">
          <span className="text-4xl font-black italic tracking-tighter uppercase font-display text-white leading-none">
            STATUS:
          </span>
          <div className="flex items-center gap-1 mt-1">
            <select
              ...
              onChange={(e) =>
                handleStatusChange(e.target.value as RegistryStatus)
              }
            ...
            >
              <option value="Available" className="bg-tactical-dark">
                AVAILABLE
              </option>
              <option value="On Patrol" className="bg-tactical-dark">
                ON PATROL
              </option>
              <option value="Responding" className="bg-tactical-dark">
                RESPONDING
              </option>
              <option value="Offline" className="bg-tactical-dark">
                OFFLINE
              </option>
            </select>
          </div>
        ...
        <div className="mt-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] opacity-60 uppercase tracking-widest font-mono font-black border-l border-tactical-cyan/30 pl-2">
                Designated Officer
              </p>
              <p className="text-sm font-bold uppercase italic tracking-tight font-display border-l border-tactical-cyan/30 pl-2 text-white">
                {profile?.name}
              </p>
            </div>
          </div>

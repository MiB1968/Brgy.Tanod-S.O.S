...
export default function GuardianAISettings({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  const [models] = useState([
    {
      ...
      name: "Qwen2 0.5B (Lightweight)",
      size: "~0.5 GB",
      speed: "Instant responses",
    },
    {
      id: "Phi-3.5-mini-instruct-q4f16_1",
      name: "Phi-3.5 Mini (High Quality)",
      size: "~2.1 GB",
      speed: "Detailed guidelines",
    },
    {
      id: "Llama-3.2-3B-Instruct-q4f16_1",
      name: "Llama 3.2 3B (Complete)",
      size: "~2.3 GB",
      speed: "Robust Tagalog support",
    },
  ]);
  ...
  const loadModel = async (modelId: string) => {
    ...
    try {
      ...
      toast(
        ...
        } is ready for offline safety duty!`
      );
    } catch (err) {
      toast(
        "Fail loading model. Make sure WebGPU is enabled, and your browser supports offline caches."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const clearCache = () => {
    if (
      confirm(
        "Clear AI model cache? This will clean up storage but requires downloading files again upon restart."
      )
    ) {
    ...
    <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-[110] p-4 backdrop-blur-sm">
      <div className="bg-[#0a1428] border border-purple-500/30 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl relative flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 space-y-6 overflow-y-auto max-h-[650px] custom-scrollbar">
          {/* Current Model */}
          <div>
            <h3 className="text-xs font-mono uppercase text-gray-400 tracking-wider mb-2">
              Active Offline Brain
            </h3>
            <div className="bg-slate-900 border border-white/5 p-3.5 rounded-2xl">
              <p className="font-semibold text-slate-100 text-sm">
                {models.find((m) => m.id === currentModel)?.name}
              </p>
              <p className="text-[9px] text-purple-400 font-mono mt-1 uppercase tracking-wider">
                WebGPU ACCELERATED
              </p>
            </div>
          </div>
          ...
          {/* Available Models */}
          <div>
            <h3 className="text-xs font-mono uppercase text-gray-400 tracking-wider mb-2.5">
              Model Selection
            </h3>
            <div className="space-y-2">
              {models.map((model) => (
                <div
                  key={model.id}
                  className="flex items-center justify-between bg-black/40 border border-white/5 p-3 rounded-2xl hover:bg-black/60 transition"
                >
                  <div>
          ...
          {isLoading && (
            <div className="bg-purple-950/20 border border-purple-500/20 p-4 rounded-2xl">
              ...
              <p className="text-[9px] text-gray-500 mt-1.5 italic">
                Stored locally in your browser sandbox
              </p>
            </div>
          )}
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-mono uppercase text-gray-300 tracking-wider">
                  Pending Offline Reports ({pendingReports.length})
                </h3>
              </div>
            ...
            {pendingReports.length > 0 ? (
              <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                {pendingReports.map((report, idx) => (
                  <div
                    key={report.id || idx}
                    className="bg-black/40 border border-white/5 p-3 rounded-2xl flex items-center justify-between gap-3 text-xs"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-mono text-[10px] text-amber-400 font-bold">
                          REP-{report.id?.slice(0, 6).toUpperCase()}
                        </span>
              ...
              <div className="bg-black/20 rounded-2xl border border-white/5 p-4 py-6 text-center text-xs text-gray-400 flex flex-col items-center justify-center gap-2">
                ...
                <span>
                  Walang pending reports. Naka-sync ang lahat ng ulat! ✅
                </span>
              </div>
            )}
          </div>

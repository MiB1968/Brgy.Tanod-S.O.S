    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex items-start justify-center p-4 pt-20">
      <div className="bg-[#16191F] border border-[#2D3139] w-full max-w-lg rounded-[40px] overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-[#2D3139] flex justify-between items-center bg-[#1A1D23]">
          <div>
            <h3 className="font-black italic text-2xl tracking-tighter uppercase text-white">
              Dispatch Responder
            </h3>
            <p className="text-[#8E9299] text-[10px] font-black uppercase tracking-[0.2em]">
              Select an available Tanod unit
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            tanods.map((tanod) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-black text-lg text-white uppercase italic tracking-tighter truncate">
                    {tanod.name}
                  </p>
                  <p className="text-xs text-[#8E9299] font-bold uppercase tracking-widest">
                    {patrols.find((p) => p.tanodId === tanod.id)?.isActive
                      ? "Online • Tactical Signal Active"
                      : "Offline • Signal Lost"}
                  </p>
                </div>
                <div
                  className={cn(
                    ...
                    selectedTanod === tanod.id
                      ? "border-[#FF4B4B] bg-white scale-110"
                      : "border-[#2D3139]"
                  )}
                />
              </button>

...
export default function ReportsView() {
  ...
  const handleGenerateBriefing = async () => {
    ...
    try {
      const summary = await guardianAI.summarizeShift(
  ...
  const generateAsset = async (
    report: any,
    type: "doc" | "slides" | "form"
  ) => {
    setGeneratingAsset(`${report.id}-${type}`);
    const toastId = toast.loading(
      `Generating Tactical ${type.toUpperCase()}...`
    );
    try {
      ...
      if (type === "doc") {
        ...
        window.open(
          `https://docs.google.com/document/d/${res.documentId}/edit`,
          "_blank"
        );
      } else if (type === "slides") {
        ...
        window.open(
          `https://docs.google.com/presentation/d/${res.presentationId}/edit`,
          "_blank"
        );
      } else if (type === "form") {
        ...
        window.open(
          `https://forms.google.com/v1/forms/${res.formId}/edit`,
          "_blank"
        );
      }
    } catch (err) {
  ...
  const filteredReports = useMemo(() => {
    ...
    if (appliedCategory !== "ALL") {
      result = result.filter(
        (r) => r.type.toLowerCase() === appliedCategory.toLowerCase()
      );
    }
    ...
    <div className="space-y-8">
      <div className="glass-panel p-8 md:p-12 rounded-[48px] border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-command">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-4 mt-3">
            <p className="text-white/30 font-bold text-xs md:text-sm uppercase tracking-[0.3em] font-mono">
              Archived Tactical Intelligence
            </p>
            <button
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          ...
          <button
            onClick={() => {
              const csv =
                "id,type,date,status,citizen,location\n" +
                filteredReports
                  .map(
                    (r) =>
                      `${r.id},${r.type},${r.date},${r.status},${r.citizen},"${r.location}"`
                  )
                  .join("\n");
        ...
        {shiftSummary && (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-2xl font-black italic tracking-tighter text-white uppercase font-mono mb-1">
                  Tactical Shift Briefing
                </h3>
                <p className="text-[10px] text-cyan-400/60 font-black uppercase tracking-widest font-mono">
                  Generated by Guardian AI • {new Date().toLocaleDateString()}
                </p>
              </div>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {filteredReports.map((report) => (
          <div
            key={report.id}
            className="glass-panel border-white/5 rounded-[40px] p-8 md:p-10 space-y-8 shadow-command relative overflow-hidden group"
          >
            ...
            <div className="flex justify-between items-start relative z-10">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h4 className="font-black text-[9px] text-emergency uppercase tracking-[0.4em] mb-2 font-mono">
                    FILE_RECORD
                  </h4>
                  <p className="text-2xl font-black text-white italic tracking-tighter uppercase font-mono leading-none">
                    {report.type}
                  </p>
                </div>
              </div>
              <span className="px-4 py-1.5 bg-success/10 border border-success/30 text-success text-[10px] font-black rounded-full uppercase font-mono italic tracking-widest">
                {report.status}
              </span>
            </div>
            ...
            <div className="space-y-6 relative z-10">
              <div className="p-6 bg-brand-bg/50 rounded-3xl border border-white/5">
                <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] mb-3 font-mono">
                  Mission Narrative
                </p>
                <div className="space-y-3">
                  <p className="font-bold text-[13px] text-white/80 font-mono italic">
                    <span className="text-white/40">
                      Alerted By Cityzen Name:
                    </span>{" "}
                    {report.citizen || "Unknown"}
                  </p>
                  <p className="font-bold text-[13px] text-white/80 font-mono italic">
                  ...
                  <div className="pt-2 border-t border-white/5 mt-2">
                    <p className="font-bold text-[13px] text-success font-mono italic">
                      <span className="text-white/40">Note Cleared by:</span>{" "}
                      {report.tanodName || "Unknown"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {report.adminOnDuty && (
                  <div className="col-span-2 p-5 bg-brand-bg/50 rounded-2xl border border-white/5">
                    <p className="text-[8px] font-black text-white/20 uppercase tracking-[0.2em] mb-2 font-mono">
                      Command Admin
                    </p>
                    <p className="text-xs font-black text-white uppercase font-mono italic tracking-tight">
                      {report.adminOnDuty}
                    </p>
                  </div>
                )}
                ...
                {report.respondedAt && report.resolvedAt && (
                  <div className="col-span-2 p-6 bg-info/5 border border-info/20 rounded-3xl flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                      <p className="text-[9px] font-black text-info uppercase tracking-[0.3em] font-mono">
                        Response Duration
                      </p>
                    ...
                    <div className="flex justify-between items-center text-[9px] text-info/50 font-black uppercase tracking-widest font-mono">
                      <span>
                        IN:{" "}
                        {new Date(report.respondedAt).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </span>
            ...
            <div className="pt-6 border-t border-white/5 flex flex-col gap-4 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] font-mono italic truncate">
                  {report.location}
                </p>
              </div>
              {report.gpsLocation && (
        ...
        {reports.length === 0 && !loading && (
          <div className="col-span-full py-40 text-center glass-panel border-dashed border-white/10 rounded-[48px]">
            ...
            <p className="text-white/20 font-black uppercase tracking-[0.4em] font-mono text-xs italic">
              Secure vault is empty. No files detected.
            </p>
          </div>
        )}
      </div>

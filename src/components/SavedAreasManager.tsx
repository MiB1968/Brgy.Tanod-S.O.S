...
import { Trash2, MapPin, Calendar, HardDrive, Map, Info, Compass } from 'lucide-react';
...
export default function SavedAreasManager() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('offline-area-downloaded', handleReload);
  }, []);

  const deleteArea = async (id: number, name: string) => {
    if (window.confirm(`⚠️ ARE YOU SURE? This will permanently delete standard cached tiles for "${name}" from your offline index store.`)) {
    ...
    <div className="relative tactical-panel border border-white/[0.08] p-6 lg:p-8 rounded-[32px] bg-brand-bg/95 backdrop-blur-2xl overflow-hidden shadow-2xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-6 mb-8">
        <div>
          ...
          <p className="text-xs text-white/40 mt-1 font-mono">
            Mamburao responder terminals cache registry for remote tactical operation.
          </p>
        </div>
        <div className="flex items-center gap-2.5 bg-[#121620] border border-white/5 px-4 py-2.5 rounded-2xl">
          ...
          <span className="text-xs font-mono font-bold text-tactical-cyan">
            {savedAreas.length} SECURE ZONE{savedAreas.length !== 1 ? 'S' : ''} INDEXED
          </span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-xs font-mono text-white/30 uppercase tracking-widest">Querying Dexie Cache DB...</p>
        </div>
      ) : savedAreas.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 px-4 border border-dashed border-white/10 rounded-[24px] bg-black/25 text-center">
          ...
          <h3 className="text-sm font-black uppercase tracking-wider text-white">No custom pre-cached regions found</h3>
          <p className="text-xs text-white/40 max-w-sm mt-2 leading-relaxed">
            Use the Live map interface and drag to highlight parts of Mamburao, or tap the <span className="text-[#FF8C00] font-black">"Cache Region"</span> control on the map toolbar to save the viewport offline.
          </p>
          <div className="mt-6 flex items-center gap-2 bg-[#121620] border border-white/5 px-4 py-2 rounded-xl text-[10px] font-mono text-white/50">
            ...
            <span>Mamburao Core Downtown area is pre-loaded automatically.</span>
          </div>
        </div>
      ) : (
              ...
              <div className="flex-1 min-w-0 pr-4">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[10px] text-white/40 font-mono mt-1">
                  <span className="flex items-center gap-1">
                    ...
                    {new Date(area.downloadedAt).toLocaleDateString('en-PH', { month: 'short', day: '2-digit', year: 'numeric' })}
                  </span>
                  <span>•</span>
                  <span className="text-tactical-cyan font-bold">{area.tileCount} Tiles stored</span>
                  <span>•</span>
                  <span className="bg-white/5 px-1.5 py-0.5 rounded text-white/60">Zoom {area.minZoom}–{area.maxZoom}</span>
                </div>
              </div>

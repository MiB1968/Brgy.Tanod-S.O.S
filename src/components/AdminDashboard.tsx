...
export default function AdminDashboard(props: {
  ...
  useEffect(() => {
    console.log(
      "[DEBUG] AdminDashboard loadStats useEffect mounted with profile role:",
      profile?.role
    );
    // Hard restrict: Resident role should never be here, and if they are, do nothing.
    if (!profile || profile.role === "resident") return;
    ...
    const loadStats = async () => {
      try {
        ...
        const uniqueTanods = Array.from(
          new Map(tanodsData.map((t: any) => [t.id, t])).values()
        ) as User[];
    ...
    try {
      ...
      if (status === "responding" || status === "resolved") {
        const responseTime =
          status === "responding"
            ? Math.floor(
                (new Date(updateData.respondedAt).getTime() -
  ...
  const handleUpdateTanodStatus = async (
    tanodId: string,
    newStatus: string
  ) => {
    try {
      ...
      await api.generic.update(`patrols/${tanodId}`, {
        ...
        status: newStatus.toLowerCase().includes("responding")
          ? "responding"
          : isActuallyOnline
          ? "patrolling"
          : "offline",
    ...
    >
      ...
      {activeBroadcast && (
        <motion.div
          ...
          className="tactical-panel border-tactical-red/30 bg-tactical-red/10 rounded-[40px] p-8 border-l-4 border-l-tactical-red shadow-[0_0_20px_var(--color-tactical-red)] overflow-hidden relative mb-6"
        >
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1">
              <h4 className="text-[10px] font-black uppercase text-tactical-red tracking-widest font-mono">
                ACTIVE SYSTEM SOS BROADCAST
              </h4>
              <p className="text-xl font-black italic tracking-tighter uppercase text-white font-display mt-1">
                "{activeBroadcast.message}"
              </p>
              <p className="text-[10px] font-bold text-white/40 mt-1 uppercase">
                INITIATED BY ADMIN: {activeBroadcast.adminName}
              </p>
            </div>
          </div>
        </motion.div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start h-full">
        ...
        {(layout === "panoramic" || layout === "standard") && (
          <div className="w-full lg:w-80 xl:w-96 space-y-6 flex-shrink-0">
            ...
            />
            <PersonnelSidebar tanods={onDutyTanods} patrols={patrols} />
          </div>
        )}
        ...
        <div className="flex-1 w-full space-y-6">
          ...
          <motion.div
            variants={itemVariants}
            className="w-full h-[600px] rounded-[32px] overflow-hidden tactical-panel border border-tactical-cyan/25 relative shadow-2xl"
          >
            <LiveMap />
          </motion.div>
          ...
          <motion.div
            variants={itemVariants}
            className="tactical-panel border-tactical-cyan/40 p-6 md:p-8 rounded-[32px] bg-tactical-dark/95 shadow-[0_0_20px_rgba(0,240,255,0.1)] relative overflow-hidden"
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 border-b border-white/5 pb-4">
              <div>
                <span className="text-[9px] font-black tracking-widest text-tactical-cyan font-mono uppercase">
                  SECURE_ROUTING_GRID
                </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 relative z-10">
              {[
                {
                  id: "map",
                  label: "Live Intel Map",
                  icon: MapIcon,
                  desc: "Active Heatmaps",
                  color:
                    "text-tactical-cyan border-tactical-cyan/10 hover:bg-tactical-cyan/5 hover:border-tactical-cyan/45 shadow-[0_0_15px_rgba(0,240,255,0.05)]",
                },
                {
                  id: "tracker",
                  label: "GPS Dispatch",
                  icon: Activity,
                  desc: "Tanod Live Tracks",
                  color:
                    "text-emerald-400 border-emerald-400/10 hover:bg-emerald-400/5 hover:border-emerald-400/45",
                },
                {
                  id: "users",
                  label: "Verifications",
                  icon: UserCheck,
                  desc: "Access Clearances",
                  color:
                    "text-amber-400 border-amber-400/10 hover:bg-amber-400/5 hover:border-amber-400/45",
                },
                {
                  id: "resident-map",
                  label: "Locator Grid",
                  icon: Eye,
                  desc: "Target Overcasts",
                  color:
                    "text-purple-400 border-purple-400/10 hover:bg-purple-400/5 hover:border-purple-400/45",
                },
                {
                  id: "roster",
                  label: "Tanod Roster",
                  icon: Shield,
                  desc: "Unit Deployments",
                  color:
                    "text-blue-400 border-blue-400/10 hover:bg-blue-400/5 hover:border-blue-400/45",
                },
                {
                  id: "schedule",
                  label: "Shift Planner",
                  icon: Calendar,
                  desc: "Patrol Schedules",
                  color:
                    "text-indigo-400 border-indigo-400/10 hover:bg-indigo-400/5 hover:border-indigo-400/45",
                },
                {
                  id: "reports",
                  label: "Threat Feed",
                  icon: AlertOctagon,
                  desc: "SOS Threat Bulletins",
                  color:
                    "text-tactical-red border-tactical-red/10 hover:bg-tactical-red/5 hover:border-tactical-red/45",
                },
                {
                  id: "records",
                  label: "Workspace Doc",
                  icon: FileText,
                  desc: "Barangay Records",
                  color:
                    "text-zinc-300 border-zinc-300/10 hover:bg-zinc-300/5 hover:border-zinc-300/45",
                },
                {
                  id: "directory",
                  label: "SOS Directory",
                  icon: PhoneCall,
                  desc: "Secure Hotlines",
                  color:
                    "text-rose-400 border-rose-400/10 hover:bg-rose-400/5 hover:border-rose-400/45",
                },
                {
                  id: "guardian",
                  label: "Guardian AI",
                  ...
                  desc: "Sound Analysis",
                  color:
                    "text-fuchsia-400 border-fuchsia-400/10 hover:bg-fuchsia-400/5 hover:border-fuchsia-400/45",
                },
                {
                  id: "ops",
                  label: "Command API",
                  ...
                  color:
                    "text-teal-400 border-teal-400/10 hover:bg-teal-400/5 hover:border-teal-400/45",
                },
                {
                  id: "logs",
                  label: "System Telemetry",
                  icon: History,
                  desc: "Raw Activity Logs",
                  color:
                    "text-slate-400 border-slate-400/10 hover:bg-slate-400/5 hover:border-slate-400/45",
                },
                {
                  id: "settings",
                  label: "Config Admin",
                  icon: SettingsIcon,
                  desc: "Engine Settings",
                  color:
                    "text-teal-400 border-teal-400/10 hover:bg-teal-400/5 hover:border-teal-400/45",
                },
              ].map((mod) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-[7px] font-mono text-white/20 font-black tracking-widest uppercase group-hover:text-white/40 transition-colors">
                        SYNC
                      </span>
                    </div>
                    ...
                    <p className="text-[8px] font-bold text-white/30 tracking-tight leading-normal mt-1 font-mono truncate">
                      {mod.desc}
                    </p>
                  </button>
                );
        ...
        {layout === "panoramic" && (
          <div className="w-full lg:w-80 xl:w-96 space-y-6 flex-shrink-0">
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <h3 className="text-xs font-black uppercase tracking-widest text-zinc-500 font-mono mb-4">
                Raw Activity Intel
              </h3>
              <TanodActivityLogs />
            </div>

...
export function SOSButtonPanel({
  isSending,
  guardianMode,
  setGuardianMode,
  onInitiateSOS,
}: SOSButtonPanelProps) {
  const { isPassiveActive, isRecording, recordingDuration } =
    useGuardianPassive(guardianMode, (type, desc) => onInitiateSOS(type, desc));
      ...
      <TacticalCard className="p-8 md:p-12 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-center justify-between mb-12 px-6 py-4 rounded-2xl bg-tactical-dark border border-tactical-cyan/30 backdrop-blur-md shadow-inner">
          <div className="flex flex-col gap-1">
            <span className="text-[9px] font-black tracking-[0.2em] text-tactical-cyan/80 uppercase">
              Guardian AI
            </span>
            <div className="flex items-center gap-2">
              <div
                className={cn(
                  ...
                  guardianMode
                    ? "bg-tactical-cyan shadow-[0_0_8px_var(--color-tactical-cyan)]"
                    : "bg-white/20"
                )}
              />
              <span
                className={cn(
                  "text-[12px] font-black tracking-wider transition-colors",
                  guardianMode ? "text-tactical-cyan" : "text-white/50"
                )}
              >
          ...
          <button
            ...
            className={cn(
              "relative w-14 h-7 rounded-full p-1 transition-all duration-300 border border-tactical-cyan/30",
              guardianMode ? "bg-tactical-cyan/20" : "bg-tactical-dark"
            )}
          >
            <div
              className={cn(
                "w-5 h-5 rounded-full bg-white transition-all shadow-[0_0_10px_white]",
                guardianMode
                  ? "translate-x-7 bg-tactical-cyan"
        ...
        {guardianMode && (
          <div className="flex items-center gap-2 px-4 py-1.5 -mt-2 mb-4 rounded-xl bg-black/30 border border-tactical-cyan/10 text-[11px] font-mono">
            ...
            />
            <span
              className={isRecording ? "text-red-400" : "text-emerald-400/80"}
            >
              {isRecording
                ? `Nire-record... ${formatDuration(recordingDuration)}`
                : "Nakikinig..."}
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-4 mt-8">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() =>
              triggerSOS("CRIME", "Criminal activity/Crime emergency reported.")
            }
            disabled={isSending}
        ...
        <VoiceSOSButton
          onSOSReady={(type, desc) => onInitiateSOS(type, desc)}
        />
        ...
        <div className="mt-12 flex items-center justify-center gap-4 bg-tactical-dark border border-tactical-cyan/20 rounded-full py-4 text-tactical-cyan/60">
          ...
          <p className="font-black tracking-[0.5em] text-[10px] font-mono uppercase">
            Tactical Emergency Signal
          </p>

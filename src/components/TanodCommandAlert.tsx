...
export default function TanodCommandAlert({
  profile,
  isTestMode,
}: {
  profile: User;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        {pendingResponse ? (
          ...
          >
            <h3 className="text-white text-xl font-black uppercase mb-4">
              Confirm {pendingResponse}?
            </h3>
            <p className="text-[#8E9299] mb-6">
              Are you sure you want to{" "}
              {pendingResponse === "accepted" ? "accept" : "decline"} this task?
            </p>
            <div className="flex gap-3">
              <button
                onClick={cancelResponse}
                className="flex-1 py-3 bg-[#252932] text-white rounded-xl"
              >
                Cancel
              </button>
              <button
                ...
                className={`flex-1 py-3 ${
                  pendingResponse === "accepted" ? "bg-green-500" : "bg-red-500"
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-[#0F1115] p-4 rounded-2xl border border-[#2D3139]">
                <p className="text-xs text-[#8E9299] uppercase font-bold tracking-widest mb-1">
                  Sector
                </p>
                ...
                <div className="mt-3">
                  ...
                  <p className="text-white font-medium text-sm">
                    {new Date(activeAlert.startTime).toLocaleTimeString()} -{" "}
                    {new Date(activeAlert.endTime).toLocaleTimeString()}
                  </p>
                </div>
